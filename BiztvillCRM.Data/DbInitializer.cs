...
public static class DbInitializer
{
    public static void Initialize(CrmDbContext db)
    {
        ...
        var meresek = new List<Meres>
        {
            ...
            new() { UgyfelId = ugyfelek[0].Id,  TelephelyId = telephelyek[0].Id,  MeresTipusId = meresTipusok[0].Id, Datum = now.AddMonths(-10), KovetkezoDatum = now.AddMonths(2),   MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Nincs rendellenesség",          Letrehozva = now.AddMonths(-10) },
            new() { UgyfelId = ugyfelek[0].Id,  TelephelyId = telephelyek[1].Id,  MeresTipusId = meresTipusok[1].Id, Datum = now.AddMonths(-4),  KovetkezoDatum = now.AddMonths(2),   MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Eltérés 0,2°C – javítva",      Letrehozva = now.AddMonths(-4)  },
            new() { UgyfelId = ugyfelek[1].Id,  TelephelyId = telephelyek[2].Id,  MeresTipusId = meresTipusok[2].Id, Datum = now.AddMonths(-8),  KovetkezoDatum = now.AddMonths(4),   MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Hitelesítési jegy kiadva",      Letrehozva = now.AddMonths(-8)  },
            new() { UgyfelId = ugyfelek[1].Id,  TelephelyId = telephelyek[3].Id,  MeresTipusId = meresTipusok[3].Id, Datum = now.AddMonths(-2),  KovetkezoDatum = now.AddMonths(1),   MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Szivárgás nem észlelhető",      Letrehozva = now.AddMonths(-2)  },
            new() { UgyfelId = ugyfelek[1].Id,  TelephelyId = telephelyek[2].Id,  MeresTipusId = meresTipusok[4].Id, Datum = now.AddMonths(-18), KovetkezoDatum = now.AddMonths(6),   MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Szigetelési ellenállás: 500 MΩ",Letrehozva = now.AddMonths(-18) },
            new() { UgyfelId = ugyfelek[2].Id,  TelephelyId = telephelyek[4].Id,  MeresTipusId = meresTipusok[0].Id, Datum = now.AddMonths(-6),  KovetkezoDatum = now.AddMonths(6),   MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Nyomáspróba átment",            Letrehozva = now.AddMonths(-6)  },
            new() { UgyfelId = ugyfelek[3].Id,  TelephelyId = telephelyek[5].Id,  MeresTipusId = meresTipusok[1].Id, Datum = now.AddMonths(-3),  KovetkezoDatum = now.AddMonths(3),   MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Pontosság ±0,1°C",              Letrehozva = now.AddMonths(-3)  },
            new() { UgyfelId = ugyfelek[3].Id,  TelephelyId = telephelyek[6].Id,  MeresTipusId = meresTipusok[2].Id, Datum = now.AddMonths(-12), KovetkezoDatum = now.AddMonths(0),   MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Kalibrálás elvégezve",          Letrehozva = now.AddMonths(-12) },
            new() { UgyfelId = ugyfelek[4].Id,  TelephelyId = telephelyek[7].Id,  MeresTipusId = meresTipusok[3].Id, Datum = now.AddMonths(-1),  KovetkezoDatum = now.AddMonths(2),   MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "CO szint rendben",              Letrehozva = now.AddMonths(-1)  },
            new() { UgyfelId = ugyfelek[4].Id,  TelephelyId = telephelyek[8].Id,  MeresTipusId = meresTipusok[0].Id, Datum = now.AddMonths(-20), KovetkezoDatum = now.AddMonths(-8),  MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Nyomáspróba sikeres",           Letrehozva = now.AddMonths(-20) },
            new() { UgyfelId = ugyfelek[4].Id,  TelephelyId = telephelyek[7].Id,  MeresTipusId = meresTipusok[3].Id, Datum = now.AddMonths(-5),  KovetkezoDatum = now.AddMonths(-2),  MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Gázérzékelő rendben",           Letrehozva = now.AddMonths(-5)  },

            // --- Hamarosan lejáró (következő dátum 30 napon belül) ---
            new() { UgyfelId = ugyfelek[5].Id,  TelephelyId = telephelyek[9].Id,  MeresTipusId = meresTipusok[4].Id, Datum = now.AddMonths(-23), KovetkezoDatum = now.AddDays(15),    MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Hamarosan megújítandó",         Letrehozva = now.AddMonths(-23) },
            new() { UgyfelId = ugyfelek[5].Id,  TelephelyId = telephelyek[9].Id,  MeresTipusId = meresTipusok[1].Id, Datum = now.AddMonths(-5),  KovetkezoDatum = now.AddDays(10),    MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "10 napon belül lejár",          Letrehozva = now.AddMonths(-5)  },
            new() { UgyfelId = ugyfelek[6].Id,  TelephelyId = telephelyek[10].Id, MeresTipusId = meresTipusok[2].Id, Datum = now.AddMonths(-11), KovetkezoDatum = now.AddDays(25),    MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "25 napon belül lejár",          Letrehozva = now.AddMonths(-11) },
            new() { UgyfelId = ugyfelek[7].Id,  TelephelyId = telephelyek[11].Id, MeresTipusId = meresTipusok[0].Id, Datum = now.AddMonths(-11), KovetkezoDatum = now.AddDays(5),     MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "5 napon belül lejár – sürgős!",  Letrehozva = now.AddMonths(-11) },

            // --- Már lejárt (következő dátum a múltban) ---
            new() { UgyfelId = ugyfelek[7].Id,  TelephelyId = telephelyek[11].Id, MeresTipusId = meresTipusok[3].Id, Datum = now.AddMonths(-4),  KovetkezoDatum = now.AddMonths(-1),  MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Lejárt – megújítás szükséges",  Letrehozva = now.AddMonths(-4)  },
            new() { UgyfelId = ugyfelek[8].Id,  TelephelyId = telephelyek[12].Id, MeresTipusId = meresTipusok[4].Id, Datum = now.AddMonths(-26), KovetkezoDatum = now.AddMonths(-2),  MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "Lejárt – ütemezni kell",        Letrehozva = now.AddMonths(-26) },
            new() { UgyfelId = ugyfelek[8].Id,  TelephelyId = telephelyek[12].Id, MeresTipusId = meresTipusok[1].Id, Datum = now.AddMonths(-8),  KovetkezoDatum = now.AddMonths(-2),  MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Megfelelő",    Megjegyzes = "2 hónapja lejárt",              Letrehozva = now.AddMonths(-8)  },
            new() { UgyfelId = ugyfelek[9].Id,  TelephelyId = telephelyek[13].Id, MeresTipusId = meresTipusok[0].Id, Datum = now.AddMonths(-15), KovetkezoDatum = now.AddMonths(-3),  MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Nem megfelelő", Megjegyzes = "Szivárgás észlelve – javítás szükséges", Letrehozva = now.AddMonths(-15) },
            new() { UgyfelId = ugyfelek[9].Id,  TelephelyId = telephelyek[14].Id, MeresTipusId = meresTipusok[2].Id, Datum = now.AddMonths(-14), KovetkezoDatum = now.AddMonths(-2),  MeresStatusz = MeresStatusz.Elvegezve, Eredmeny = "Nem megfelelő", Megjegyzes = "Eszköz csere folyamatban",      Letrehozva = now.AddMonths(-14) },

            // --- Tervezett (jövőbeli) ---
            new() { UgyfelId = ugyfelek[0].Id,  TelephelyId = telephelyek[0].Id,  MeresTipusId = meresTipusok[3].Id, Datum = now.AddMonths(1),   KovetkezoDatum = now.AddMonths(4),   MeresStatusz = MeresStatusz.Tervezett,  Eredmeny = null,           Megjegyzes = "Ütemezett gázszivárgás vizsgálat", Letrehozva = now.AddDays(-5)    },
            new() { UgyfelId = ugyfelek[1].Id,  TelephelyId = telephelyek[2].Id,  MeresTipusId = meresTipusok[1].Id, Datum = now.AddMonths(2),   KovetkezoDatum = now.AddMonths(8),   MeresStatusz = MeresStatusz.Tervezett,  Eredmeny = null,           Megjegyzes = "Éves kalibráció",               Letrehozva = now.AddDays(-3)    },
            new() { UgyfelId = ugyfelek[2].Id,  TelephelyId = telephelyek[4].Id,  MeresTipusId = meresTipusok[0].Id, Datum = now.AddMonths(3),   KovetkezoDatum = now.AddMonths(15),  MeresStatusz = MeresStatusz.Tervezett,  Eredmeny = null,           Megjegyzes = "Tervezett nyomáspróba",         Letrehozva = now.AddDays(-2)    },

            // --- Folyamatban ---
            new() { UgyfelId = ugyfelek[4].Id,  TelephelyId = telephelyek[7].Id,  MeresTipusId = meresTipusok[4].Id, Datum = now.AddDays(-1),    KovetkezoDatum = null,               MeresStatusz = MeresStatusz.Folyamatban, Eredmeny = null,          Megjegyzes = "Vizsgálat folyamatban",         Letrehozva = now.AddDays(-1)    },
            new() { UgyfelId = ugyfelek[5].Id,  TelephelyId = telephelyek[9].Id,  MeresTipusId = meresTipusok[2].Id, Datum = now,                KovetkezoDatum = null,               MeresStatusz = MeresStatusz.Folyamatban, Eredmeny = null,          Megjegyzes = "Ma megkezdett mérés",           Letrehozva = now                },

            // --- Elutasítva ---
            new() { UgyfelId = ugyfelek[4].Id,  TelephelyId = telephelyek[8].Id,  MeresTipusId = meresTipusok[3].Id, Datum = now.AddMonths(-3),  KovetkezoDatum = null,               MeresStatusz = MeresStatusz.Elutasitva, Eredmeny = "Elutasítva", Megjegyzes = "Hiányos dokumentáció",          Letrehozva = now.AddMonths(-3)  },
            new() { UgyfelId = ugyfelek[7].Id,  TelephelyId = telephelyek[11].Id, MeresTipusId = meresTipusok[0].Id, Datum = now.AddMonths(-7),  KovetkezoDatum = null,               MeresStatusz = MeresStatusz.Elutasitva, Eredmeny = "Elutasítva", Megjegyzes = "Eszköz nem hozzáférhető",       Letrehozva = now.AddMonths(-7)  },
            new() { UgyfelId = ugyfelek[9].Id,  TelephelyId = telephelyek[13].Id, MeresTipusId = meresTipusok[1].Id, Datum = now.AddMonths(-9),  KovetkezoDatum = null,               MeresStatusz = MeresStatusz.Elutasitva, Eredmeny = "Elutasítva", Megjegyzes = "Ügyfél lemondta",               Letrehozva = now.AddMonths(-9)  },
            new() { UgyfelId = ugyfelek[9].Id,  TelephelyId = telephelyek[14].Id, MeresTipusId = meresTipusok[4].Id, Datum = now.AddMonths(-11), KovetkezoDatum = null,               MeresStatusz = MeresStatusz.Elutasitva, Eredmeny = "Elutasítva", Megjegyzes = "Nem megfelelő feltételek",      Letrehozva = now.AddMonths(-11) },
        };
